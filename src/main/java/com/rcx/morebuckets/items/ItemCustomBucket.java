package com.rcx.morebuckets.items;
...
public class ItemCustomBucket extends UniversalBucket implements IFluidContainerItem {
	...
	public ItemCustomBucket(String name, ResourceLocation item, boolean canCarryHot, boolean canBreak, int materialColor) {
		...
		for(Fluid fluid : FluidRegistry.getRegisteredFluids().values()) {
			...
			if(!fluid.getName().equals("milk")) {
				...
				if(fill(stack, fs, true) == fs.amount) {
					MoreBuckets.filledBuckets.add(stack);
				}
			}
		}
		// special fluids
		for(SpecialFluid fluid : SpecialFluid.values()) {
			if(fluid.show()) {
				MoreBuckets.filledBuckets.add(new ItemStack(this, 1, fluid.getMeta()));
			}
		}
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		if(fluidStack == null && !hasSpecialFluid(stack)) {
			return I18n.translateToLocal("item.morebuckets.bucket_empty.name").replace("@material", material);
		}

		if(hasSpecialFluid(stack)) {
			return I18n.translateToLocal("item.morebuckets.bucket.name").replace("@material", material).replace("@liquid", I18n.translateToLocal("liquid." + getSpecialFluid(stack).getName() + ".name"));
		} else {
			fluid = fluidStack.getLocalizedName();
		}

		if(I18n.canTranslate("item.morebuckets.bucket." + materialName + "." + fluidStack.getFluid().getName() + ".name")) {
			return I18n.translateToLocal("item.morebuckets.bucket." + materialName + "." + fluidStack.getFluid().getName() + ".name");
		}

		return I18n.translateToLocal("item.morebuckets.bucket.name").replace("@material", material).replace("@liquid", fluid);
	}
